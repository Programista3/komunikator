...
exports.userExists = function(data, callback) {
	pool.getConnection(function(err, connection) {
		...
		if('login' in data && 'password' in data) {
			connection.query('SELECT * FROM users WHERE (username = ? OR email = ?);', [data.login, data.login], function(error, results, fields) {
				connection.release();
				if(error) {
	...
	pool.getConnection(function(err, connection) {
		...
		bcrypt.genSalt(10, function(err, salt) {
			bcrypt.hash(user.password, salt, function(err, hash) {
				connection.query('INSERT INTO users (firstname, lastname, username, email, password, register_date) VALUES (?, ?, ?, ?, ?, NOW());', [user.firstname, user.lastname, user.username, user.email, hash], function(error, results, fields) {
					connection.release();
					if(error) throw error;
					callback();
				});
